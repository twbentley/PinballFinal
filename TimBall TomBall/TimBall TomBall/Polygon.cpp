...
Polygon::Polygon()
{
 ...
 //       x = static_cast<GLfloat>( 100 * radius * cos(theta + PI / 2));
 //       y = static_cast<GLfloat>( 100 * radius * sin(theta + PI / 2));
 //       
	//	points[i] = Vector4(x, y, 0, 0);
 //   }

	points[0] = Vector4(-100.0f, -100.0f, 0, 1);
	points[1] = Vector4(-100.0f, 100.0f, 0, 1);
	points[2] = Vector4(100.0f, 100.0f, 0, 1);
	points[3] = Vector4(100.0f, -100.0f, 0, 1);
...
void Polygon::Draw(ColorShader* shader, Matrix4* worldMatrix, Matrix4* viewMatrix, Matrix4* projectionMatrix)
{
	...
	temp[3] = Vector4( *worldMatrix * points[3] );
